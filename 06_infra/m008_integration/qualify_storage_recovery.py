"""
## qualify_storage_recovery.py

## Descripción
Reserva una cualificación Linux de seis contratos de recuperación, sin ciencia.
...
TEST='06_infra/m008_integration/test_storage_recovery.py'
NAMES=('single_walk','limits','links_and_errors','exact_bridge','round_reservations','reader_without_execution')
...
def main():
    ...
    marker=ROOT/'local_state/m008-s02-storage-qualification.json';q.require(not marker.exists(),'qualification consumed')
    ...
    out=Path(tempfile.mkdtemp(prefix='s02-storage-qualification-',dir=parent))
    ...
    try:
        ...
        names={e['path'] for e in bridge['after']}|{TEST,'06_infra/m008_integration/qualify_storage_recovery.py',v.RECOVERY_SCOPE,v.RECOVERY_BRIDGE}
        ...
        argv=[sys.executable,'-B',str(out/'snapshot/06_infra/m008_integration/qualify_storage_recovery.py'),'--worker',str(out)]
        ...
        d=q.launch(argv,out,token,'linux',600);q.require(d['status']=='finished' and d['exit']==0,'qualification failed/unknown; stop')
        q.require(v.junit_ids([out/'evidence/junit.xml'])==sorted(IDS),'six exact IDs')
        ...
        result={'ok':True,'tests':6,'required':IDS,'fits':0,'builds':0,'sources':sources,'seconds':d['seconds'],'invocation':token,'artifacts':artifacts,'rss_peak':'unknown','scratch_peak':'unknown','tokens':'unknown'}
        q.save(out/'result.json',result);q.save(ROOT/v.RECOVERY_REPORT,result);print(json.dumps({'ok':True,'tests':6,'fits':0,'seconds':d['seconds']}));return 0
