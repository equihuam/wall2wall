...
def predict_raster(run_dir, alignment_manifest, output_dir, *, trusted=False, window_size=512, batch_size=65536):
    ...
    with rasterio.Env(GDAL_CACHEMAX=CACHE_BYTES, GDAL_NUM_THREADS="1", GDAL_TIFF_INTERNAL_MASK=True,
        ...
        with rasterio.open(pending, "w", driver="GTiff", width=grid["width"], height=grid["height"],
                           count=1, dtype="float32", nodata=float("nan"), crs=grid["crs"],
                           transform=Affine(*grid["transform"]), tiled=True, blockxsize=256, blockysize=256,
                           compress="DEFLATE", BIGTIFF=bigtiff, NUM_THREADS="1") as dest:
